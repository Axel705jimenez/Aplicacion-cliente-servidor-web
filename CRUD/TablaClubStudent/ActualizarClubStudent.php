<!DOCTYPE html>
<html>
<title>Update ClubStudent</title>
<head>
            ...
            <div class="col-lg-8 col-xl-6">
                <div class="card rounded-3">
                    <div class="card-body p-4 p-md-5">
                        <h3 class="mb-4 pb-2 pb-md-0 mb-md-5 px-md-2">Update ClubStudent</h3>
                        <?php
                        require_once('../config.inc.php');

                        // Obtener el ID del ClubStudent
                        ...
                        // Consulta para obtener los datos del ClubStudent
                        $consulta = "SELECT * FROM ClubStudent WHERE idClubStudent = $idClubStudent ";
                        $resultado = $conn->query($consulta);

                        // Verificar si se encontraron resultados
                        if ($resultado->num_rows > 0) {
                            // Obtener los datos del ClubStudent
                            $registro = $resultado->fetch_assoc();

                            // Mostrar el formulario con los datos del ClubStudent
                            echo '<form action="ModificarClubStudent.php" method="post">';
                            ...
                            echo '<input type="hidden" name="idClubStudent" value="' . $idClubStudent  . '"/>';
                            echo '<button type="submit" class="btn btn-success btn-lg mb-1">Actualizar</button>';
                            echo '</form>';
                        } else {
                            echo '<p>No se encontraron datos para el ClubStudent seleccionado.</p>';
                        }
